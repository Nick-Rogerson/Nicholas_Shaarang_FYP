...
## dataframe
df=pd.read_csv('/home/nrogerson/Documents/Nicholas_Shaarang_FYP/Outdoor Data/data.csv')

# location of original and mask image
img_fol='/home/nrogerson/Documents/Nicholas_Shaarang_FYP/Outdoor Data/data/all/JPEGImages'
mask_fol='/home/nrogerson/Documents/Nicholas_Shaarang_FYP/Outdoor Data/data/all/SegmentationClassPNG'
ckpt_path='/home/nrogerson/Documents/Nicholas_Shaarang_FYP/Training Attempt/New model/Outdoor.pth'
base_path='/home/nrogerson/Documents/Nicholas_Shaarang_FYP/Outdoor Data/'
...
for i, batch in enumerate(test_dataloader):
    if i < 30:
        ...
        mask_bin = np.squeeze(np.where(mask_target > 0,1,0))
        pred_bin = np.squeeze(np.where(np.squeeze(batch_preds) > 0.5,1,0))
        accuracy = (np.sum(np.where(mask_bin == pred_bin,1,0))/mask_bin.size)*100
        print(str(np.sum(np.where(mask_bin == pred_bin,1,0))) + " pixels out of " + str(mask_bin.size) + " pixels matched the target mask")
        print(accuracy)
        plt.figtext(0.2,0.9,"Image " + str(filenames[i]) + " took " + str(round(inf_time,4)) + "s to infer with an accuracy of " + str(round(accuracy,2)) + "% of pixels matching", fontsize=18)
        ax[0,0].imshow(mask_bin,cmap='gray')
        ax[0,0].set_title('Original Mask')
        ax[0,1].imshow(pred_bin,cmap='gray')
        ax[0,1].set_title('Predicted Mask')
        ax[1,0].imshow(image)
        ax[1,0].set_title('Original Image')
        overlay(pred_bin,image,ax[1,1])
